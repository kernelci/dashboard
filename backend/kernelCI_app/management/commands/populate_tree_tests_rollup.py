...
def _merge_rollup(
    ...
    for key, data in chunk_rollup.items():
        if key in accumulator:
            ...
            existing["pass_tests"] += data["pass_tests"]
            existing["fail_tests"] += data["fail_tests"]
            existing["skip_tests"] += data["skip_tests"]
            existing["error_tests"] += data["error_tests"]
            existing["miss_tests"] += data["miss_tests"]
            existing["done_tests"] += data["done_tests"]
            existing["null_tests"] += data["null_tests"]
            existing["total_tests"] += data["total_tests"]
        else:
            accumulator[key] = data.copy()
...
class Command(BaseCommand):
    ...
    def _process_checkout(
        ...
        for test_chunk in _chunks(tests_qs.iterator(chunk_size=batch_size), batch_size):
            converted = [convert_test(t) for t in test_chunk]
            issues_map = fetch_test_issues([t.id for t in test_chunk])
