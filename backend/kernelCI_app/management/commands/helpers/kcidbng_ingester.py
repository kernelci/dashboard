...
def move_file_to_failed_dir(full_filepath: str, filename: str, failed_dir: str) -> None:
    try:
        os.rename(full_filepath, os.path.join(failed_dir, filename))
...
def process_file(
    ...
    if "error" in metadata:
        try:
            move_file_to_failed_dir(file.path, file.name, failed_dir)
        except Exception:
            pass
        return False
