...
def _process_build_status(
    ...
    to_process = ProcessedListingItems(
        listing_item_key=build_listing_key, checkout_id=build.checkout.id
    )
...
def _collect_tree_listing_keys(
...
) -> tuple[set[bytes], dict[str, bytes], dict[str, bytes]]:
    """
    Uses the ready_tests and ready_builds to generate the
    hash keys used in the ProcessedListingItems table.

    Returns a set with all keys to check,
    and two dicts with test/build ids mapped to their keys.
    """
    keys_to_check: set[bytes] = set()
    test_keys_by_id: dict[str, bytes] = {}
    build_keys_by_id: dict[str, bytes] = {}
    ...
    for build in ready_builds:
        ...
        keys_to_check.add(key)
        build_keys_by_id[build_id] = key

    return keys_to_check, test_keys_by_id, build_keys_by_id
...
def aggregate_tree_listing(
    ...
    keys_to_check, test_keys_by_id, build_keys_by_id = _collect_tree_listing_keys(
        ready_tests,
        test_builds_by_id,
        ready_builds,
    ...
    for test in ready_tests:
        test_checkout_id = test_builds_by_id[test.build_id].checkout_id
        try:
            status_record = tree_listing_data[test_checkout_id]
        except KeyError:
            test_checkout = test_builds_by_id[test.build_id].checkout
            status_record = _init_tree_listing_record(checkout=test_checkout)
            tree_listing_data[test_checkout_id] = status_record

        _process_test_status(
            test=test,
            test_listing_key=test_keys_by_id[test.test_id],
    ...
    for build in ready_builds:
        build_checkout_id = build_checkouts_by_id[build.build_id].id
        try:
            status_record = tree_listing_data[build_checkout_id]
        except KeyError:
            build_checkout = build_checkouts_by_id[build.build_id]
            status_record = _init_tree_listing_record(checkout=build_checkout)
            tree_listing_data[build_checkout_id] = status_record

        _process_build_status(
            build=build,
            build_listing_key=build_keys_by_id[build.build_id],
            status_record=status_record,
            existing_processed=existing_processed,
            new_processed_entries=new_processed_entries,
        )
...
class Command(BaseCommand):
    ...
    def _process_tree_listing(
        ...
        with connection.cursor() as cursor:
            cursor.executemany(
                ...
                build_pass = tree_listing.build_pass + EXCLUDED.build_pass,
                build_failed = tree_listing.build_failed + EXCLUDED.build_failed,
                build_inc = tree_listing.build_inc + EXCLUDED.build_inc,
                boot_pass = tree_listing.boot_pass + EXCLUDED.boot_pass,
                boot_failed = tree_listing.boot_failed + EXCLUDED.boot_failed,
                boot_inc = tree_listing.boot_inc + EXCLUDED.boot_inc,
                test_pass = tree_listing.test_pass + EXCLUDED.test_pass,
                test_failed = tree_listing.test_failed + EXCLUDED.test_failed,
                test_inc = tree_listing.test_inc + EXCLUDED.test_inc
                """,
                values,
            )
    ...
    def _get_ready_builds(
    ...
    ) -> tuple[Optional[list[PendingBuilds]], dict[str, Checkouts], Optional[str], int]:
        """
        ...
        Note:
            - Returns (None, last_processed_build_id, 0) if there are no pending builds.
            - Returns ([], last_processed_build_id, skipped_count) if no checkouts are found
    ...
    def _get_ready_tests(
    ...
    ) -> tuple[Optional[list[PendingTest]], dict[str, Builds], Optional[str], int]:
        """
        ...
        Note:
            - Returns (None, {}, last_processed_test_id, 0) if there are no pending tests.
            - Returns ([], {}, last_processed_test_id, skipped_count) if no builds are found
        ...
        if len(pending_tests_batch) == 0:
            out("No pending tests found, exiting batch")
            return (None, test_builds_by_id, last_processed_test_id, skipped_no_build)
    ...
    def process_pending_batch(self, batch_size: int) -> int:
        ...
        while True:
            with transaction.atomic():
                ...
                self._process_tree_listing(
                    tree_listing_data, new_processed_entries_tree
                )
                self._process_new_processed_entries(new_processed_entries_tree)

                tests_count = self._delete_ready_tests(ready_tests)
                builds_count = self._delete_ready_builds(ready_builds)
