...
def test_tree_details_boots(
    ...
    response = client.get_tree_details_boots(tree_id=id, query=query, filters=filters)
...
def test_tree_details_boots_direct(
    pytestconfig,
    base_tree: dict,
    status_code: HTTPStatus,
    has_error_body: bool,
):
    _, params, direct_params = base_tree.values()

    query = direct_params["query"]
    path_params = direct_params["path"]
    filters = params.get("filters")

    response = client.get_tree_details_specific_direct(
        base_path="treeDetailsBootsDirectView",
        query=query,
        filters=filters,
