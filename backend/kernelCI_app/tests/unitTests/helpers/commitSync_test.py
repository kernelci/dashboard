...
class TestSyncCommitsCommand:
    def test_dry_run_flag_reaches_helper(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kernelCI_app.management.commands.sync_commits.sync_commit_metadata",
            fake_sync,
        )
        with override_settings(GIT_MIRROR_DIR="/tmp/mirror"):
            call_command("sync_commits", "--dry-run")
        assert seen["dry_run"] is True
        assert seen["skip_fetch"] is False
