...
from .fixtures import TREE_TEST_DATA, HARDWARE_PLATFORM_DATA
...
class Checkout:
    ...
    def get_all_checkout_ids(cls):
        """Get all checkout IDs from TREE_TEST_DATA."""
        return list(TREE_TEST_DATA.keys())

    @classmethod
    def is_known_checkout(cls, checkout_id: str):
        """Check if a checkout ID exists in TREE_TEST_DATA."""
        return checkout_id in TREE_TEST_DATA

    @classmethod
    def get_hardware_platform_from_data(cls, checkout_id: str):
        """Get hardware platform from TREE_TEST_DATA or HARDWARE_PLATFORM_DATA."""
        checkout_data = TREE_TEST_DATA.get(checkout_id)
        if checkout_data and "hardware_platform" in checkout_data:
            return checkout_data.get("hardware_platform")
        return HARDWARE_PLATFORM_DATA.get(checkout_id)
