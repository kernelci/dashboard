...
class TreeCommitsHistoryList(BaseTreeCommitsHistory):
    ...
    def aggregate_commits(self, commit_hashes: list[str], instances: list[dict]):
        ...
        for instance in instances:
            ...
            compatibles = set(
                chain.from_iterable(
                    filter(None, instance["environment_compatible"] or [])
                )
            )
    ...
    def get(self, request: HttpRequest) -> Response:
        ...
        instances = get_tree_commit_history_hashes_aggregated(
        ...
        results = self.aggregate_commits(commit_hashes, instances)
