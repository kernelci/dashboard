...
interface IHardwareWithError
  extends Pick<TTreeTestsData, 'environmentCompatible'> {
  title: IBaseCard['title'];
}

const HardwareWithError = ({
  environmentCompatible,
  title,
}: IHardwareWithError): JSX.Element => {
  return (
    <BaseCard
      title={title}
      content={
          ...
          <DumbListingContent>
            {environmentCompatible &&
              Object.entries(environmentCompatible).map(
                ([platformWithErrorItem, platformWithErrorCount]) => {
                  return (
                    <ListingItem
                      hasBottomBorder
                      key={platformWithErrorItem}
                      errors={platformWithErrorCount}
...
export const MemoizedHardwareWithError = memo(HardwareWithError);
