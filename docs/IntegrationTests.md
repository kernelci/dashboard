...
If the checkout is related to specific hardware, also add it to `HARDWARE_PLATFORM_DATA`:

```python
HARDWARE_PLATFORM_DATA = {
    "checkout_id": "arm64,board",
    # ... other mappings
}
```
...
# Also add to HARDWARE_PLATFORM_DATA
HARDWARE_PLATFORM_DATA = {
    "raspberry_pi_001": "raspberry-pi,4-model-b",
    # ... existing mappings
}
```
...
"build_id": {
    ...
    "status": "PASS",                       # Optional: Build status (PASS/FAIL/ERROR)
...
```python
"test_id": {
    "status": "PASS",                       # Optional: Test status (PASS/FAIL/ERROR/SKIP)
...
```bash
TEST_BASE_URL=http://localhost:8001 poetry run pytest -m integration --use-local-db backend/kernelCI_app/tests/integrationTests/treeListing_test.py
```
